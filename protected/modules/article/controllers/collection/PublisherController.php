...
class PublisherController extends Controller
{
	...
	public function actionSuggest($limit=10) 
	{
		if(Yii::app()->request->isAjaxRequest) {
			if(isset($_GET['term'])) {
				...
				$model = ArticleCollectionPublisher::model()->findAll($criteria);
	...
	{
		$model=new ArticleCollectionPublisher;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['ArticleCollectionPublisher'])) {
			$model->attributes=$_POST['ArticleCollectionPublisher'];
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				ArticleCollectionPublisher::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				ArticleCollectionPublisher::model()->deleteAll($criteria);
			}
		}
	...
	{
		$model = ArticleCollectionPublisher::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404, Yii::t('phrase', 'The requested page does not exist.'));
		return $model;
	}
